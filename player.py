    ...
    def generate_move(self, board: Block) -> \
        ...
        if not self._proceed:
            ...
            for move in valid_moves_lst:
                if len(n_valid_moves) == self._num_test:
                    break
                board_temp = random_block.create_copy()
                if _is_valid_move(self, board_temp, move):
                    n_valid_moves.append(move)

            # apply moves in n_valid_moves and see if score goes higher than
            # original score if it does, return the move if it doesn't return
            # pass
            best_move = self._get_best_move(n_valid_moves, board,
                                            original_score, random_block)

        self._proceed = False
        board_selected = _get_block(board, random_block.position,
                                    random_block.level)
        if best_move is None or self._num_test == 0:
            return PASS, board_selected
        else:
            return best_move, board

    def _get_best_move(self, n_valid_moves: list[Action], board: Block,
                       original_score: int,
                       random_block: Block) -> Action | None:
        """ Get the best move.
        """
        best_move = None
        for move in n_valid_moves:
            board_temp_valid = board.create_copy()
            rand_board_valid = _get_block(board_temp_valid,
                                          random_block.position,
                                          random_block.level)
            if move.short_name == 'paint':
                move.apply(rand_board_valid, {'colour': self.goal.colour})
                score = self.goal.score(board_temp_valid) - move.penalty
                if score > original_score:
                    best_move = move
            else:
                move.apply(rand_board_valid, {})
                score = self.goal.score(board_temp_valid) - move.penalty
                if score > original_score:
                    best_move = move
        return best_move
